...
function showpageposts(_0xef2cx8) {
    ...
    for (var _0xef2cxe = 0; _0xef2cxe < postsperpage; _0xef2cxe++) {
        ...
        _0xef2cxd += '<h6><a href="' + _0xef2cxb + '" target="_blank">' + _0xef2cxa + '</a></h6>';
        _0xef2cxd += '<div class="iteminside"><a href="' + _0xef2cxb + '" target="_blank"><img src="' + _0xef2cxc + '" /></a>';
...
function pagination() {
    ...
    output = '';
    if (actualhalaman > 1) {
        output += '<a class="prevjson" href="javascript:incluirscript(' + parseInt(actualhalaman - 1) + ')">' + prevText + '</a>';
    } else {
        output += '<span class="prevjson hidden">' + prevText + '</span>';
    };
    if (pagernum < (maxhalaman + 1)) {
        for (countP = 1; countP <= pagernum; countP++) {
            if (countP == actualhalaman) {
        ...
        if (pagernum > (maxhalaman - 1)) {
            if (actualhalaman < minhalaman) {
                for (countP = 1; countP < (maxhalaman - 2); countP++) {
                    if (countP == actualhalaman) {
                        output += '<span class="actual">' + countP + '</span>';
                    } else {
                        output += '<a href="javascript:incluirscript(' + countP + ')">' + countP + '</a>';
                ...
                if (pagernum - (minhalaman - 1) > actualhalaman && actualhalaman > (minhalaman - 1)) {
                    output += '<a href="javascript:incluirscript(1)">1</a>';
                    output += '<a href="javascript:incluirscript(2)">2</a>';
                    output += ' ... ';
                    for (countP = actualhalaman - 2; countP <= actualhalaman + 2; countP++) {
                        if (countP == actualhalaman) {
                            output += '<span class="actual">' + countP + '</span>';
                        } else {
                            output += '<a href="javascript:incluirscript(' + countP + ')">' + countP + '</a>';
                    ...
                    output += '<a href="javascript:incluirscript(2)">2</a>';
                    output += ' ... ';
                    for (countP = pagernum - (minhalaman + 1); countP <= pagernum; countP++) {
                        if (countP == actualhalaman) {
                            output += '<span class="actual">' + countP + '</span>';
                        } else {
                            output += '<a href="javascript:incluirscript(' + countP + ')">' + countP + '</a>';
    ...
    if (actualhalaman < countP - 1) {
        output += '<a class="nextjson" href="javascript:incluirscript(' + parseInt(actualhalaman + 1) + ')">' + nextText + '</a>';
    } else {
        output += '<span class="nextjson hidden">' + nextText + '</span>';
    };
    document['getElementById']('pagination')['innerHTML'] = output;
    var _0xef2cx1c = (actualhalaman * postsperpage) - (postsperpage - 1);
    var _0xef2cx1d = actualhalaman * postsperpage;
...
document['write']('<a title="Denzdii Gustiana" style="display:block !important;visibility:visible !important;opacity:1 !important;position:absolute;bottom:10px;right:14px;font:bold 8px Arial,Sans-Serif!important;color:#666;position;text-shadow:0px 1px 0px rgba(255,255,255,0.1);text-decoration:none;" href="http://under-88.blogspot.com/" target="_blank">Link</a></div></div>');