...
;$(function(){
	...
	$.tzineGET = function(subact,data,func){
		...
		$.get(ajxURL,{action:'tzine_ajax',subaction:subact,data:data},func,'json');	
	}
	
	$.tzinePOST	= function(subact,data,func){
		...
		$.post(ajxURL+'?action=tzine_ajax&subaction='+subact,{data:data},func,'json');
	}
	...
	if($('#slider').length)
	{
		$('#slider').nivoSlider({
			...
			afterChange: function(){
				...
				$('#sliderThumbnails a').removeClass('active').eq(index).addClass('active');
			}
		});
		
		var nivoControls = $('#slider a.nivo-control');	
		
		$('#sliderThumbnails a').each(function(i){
			var elem = $(this);
			...
			var span = $('<span class="pointy"><span class="pointyShadow"></span><span class="pointyFrontTip"></span></span>');
			elem.prepend(span);
	...
	$('#siteSearchInput').defaultText('Search the site');

	
//	$('#s').focus();
	
	$('#relatedSection h4 span').click(function(){
		
		trackEvent('Click', 'Show Related');
		
		var ul = $('#relatedSection ul');
		ul.fadeTo(250,0.6);
		$.tzineGET('reloadShuffleRelated',function(response){
			ul.replaceWith(response.html);
		});
	});
	
	$('.fvCommunityNewsList a').click(function(){
		trackEvent('Click', 'Community News',this.href);
	});
	...
	if(window.FB)
	{
		...
		FB.Event.subscribe("auth.login", function(response)
		{
			...
			$.tzineGET('fbLogin',function(response){
				if(response.status){
					if(response.action && response.action.name == 'insert'){
						$('<span>',{html:response.action.html}).hide().appendTo(response.action.selector).fadeIn();
					}
				}
			});
		});
	...
	$('.cTip').colorTip({timeout:250});
	
	/*
	$.getJSON('http://demo.tutorialzine.com/2010/03/who-is-online-widget-php-mysql-jquery/who-is-online/online.php?callback=?',function(msg){
		//$('#footerLinks').append('| '+msg.online+' Currently Online');
	});
	*/
	
	try{
		SyntaxHighlighter.config.clipboardSwf = '/wp-content/plugins/easy-google-syntax-highlighter/scripts/clipboard.swf';
	...
	$('#bsaADS a:not(.adhere)').live('click',function(){
		trackEvent('Click','BuySellAds Banner Click',this.title);
	});
	
	$('a.adhere').live('click',function(){
		trackEvent('Click','BuySellAds Advertise Here Click');
	});
	
	$('#coolAds a').click(function(){
		trackEvent('Banner Click', $(this).attr('href').match(/products\/(\w+)/)[1]+' banner');
	});
	
	$('#wideZineBanner a').click(function(){
		trackEvent('Banner Click', 'wide banner');
	});
		
	$('#downloadDemo .download').click(function(){
		trackEvent('Download', 'DemoFiles', this.href);
	});
	
	$('#downloadDemo .demo').click(function(){
		trackEvent('Visit', 'Demo', this.href);
	});
	
	$('#postBody a').live('click',function(){
		trackEvent('Click', 'ArticleLink', this.href);
	});
	
	$('div.textads a').live('click',function(){
		trackEvent('Click', 'TextAd', this.href);
	});

	$('ul.tweetFavList a').live('click',function(){
		trackEvent('Click', 'Tweet Sidebar', this.href);
	});
	
	$('a.goFullScreen').live('click',function(){
		trackEvent('Click', 'Go Full Screen');
	});
	
	/* Randomizing the text ads */
	...
	var textAdsLI = $('#sidebar div.textads li');
	var textAds = [];
	
	textAdsLI.each(function(){
		textAds.push($(this).html());
	});
	
	textAds = shuffle(textAds);
	
	for(var i=0;i<textAds.length;i++){
		textAdsLI.eq(i).html(textAds[i]);
	}
	...
	$('#tweetToDownload a.tweet,#tweetToDownload a.inactive').live('click',function(){
		return false;
	});
	
	if(window.postData)
	$('#tweetToDownload a.tweet').tweetAction({
		text:		postData.title+' -',
		url:		postData.link,
		via:		'tutorialzine',
		...
		$('#downloadPSD')
				...
				.attr('href',$('#psdURL').html())
				.click(function(){
					trackEvent('Download', 'PSD', this.href);
				});
	});
	
	// The Read FullScreen button
	
	if($.support.fullscreen){
		
		$('html').addClass('supportsFullScreen');
		
		$('a.goFullScreen').live('click',function(e){
			e.preventDefault();
			$('#main').fullScreen();
		});
	}
...
// Tweet action plugin
(function(a){var b=null;a.fn.tweetAction=function(c,d){c=a.extend({url:window.location.href},c);return this.click(function(k){if(b){k.preventDefault();return}var h=550,f=350,j=(window.screen.height-f)/2,i=(window.screen.width-h)/2;var g=["scrollbars=yes","resizable=yes","toolbar=no","location=yes","width="+h,"height="+f,"left="+i,"top="+j].join(",");b=window.open("http://twitter.com/intent/tweet?"+a.param(c),"TweetWindow",g);(function l(){try{if(!b||b.closed){throw"Closed!"}else{setTimeout(l,100)}}catch(m){b=null;d()}})();k.preventDefault()})}})(jQuery);

// Colortip Plugin
$.fn.colorTip=function(settings){var defaultSettings={color:'yellow',timeout:500};var supportedColors=['red','xBlack','blue','white','yellow','black'];settings=$.extend(defaultSettings,settings);return this.each(function(){var elem=$(this);if(!elem.attr('title'))return true;var scheduleEvent=new eventScheduler();var tip=new Tip(elem.attr('title'));elem.append(tip.generate()).addClass('colorTipContainer');var hasClass=false;for(var i=0;i<supportedColors.length;i++){if(elem.hasClass(supportedColors[i])){hasClass=true;break;}}if(!hasClass){elem.addClass(settings.color);}elem.hover(function(){tip.show();scheduleEvent.clear();},function(){scheduleEvent.set(function(){tip.hide();},settings.timeout);});elem.removeAttr('title');});};function eventScheduler(){};eventScheduler.prototype={set:function(func,timeout){this.timer=setTimeout(func,timeout);},clear:function(){clearTimeout(this.timer);}};function Tip(txt){this.content=txt;this.shown=false;};Tip.prototype={generate:function(){return this.tip||(this.tip=$('<span class="colorTip">'+this.content+'<span class="pointyTipShadow"></span><span class="pointyTip"></span></span>'));},show:function(){if(this.shown)return;this.tip.css('margin-left',-this.tip.outerWidth()/2).fadeIn('fast');this.shown=true;},hide:function(){this.tip.fadeOut();this.shown=false;}}
...
// Todo: move it in a different file

SyntaxHighlighter.brushes.CoffeeScript=function(){var csKeywords,jsKeywords,keywords;jsKeywords='if else new return try catch finally throw break continue for in while delete instanceof typeof switch super extends class case default do function var void with const let debugger enum export import native __extends __hasProp';csKeywords='then unless and or is isnt not of by where when until';keywords=jsKeywords+' '+csKeywords;this.regexList=[{regex:SyntaxHighlighter.regexLib.singleLinePerlComments,css:'comments'},{regex:/\#\#\#[\s\S]*?\#\#\#/gm,css:'comments'},{regex:SyntaxHighlighter.regexLib.multiLineDoubleQuotedString,css:'string'},{regex:SyntaxHighlighter.regexLib.doubleQuotedString,css:'string'},{regex:SyntaxHighlighter.regexLib.singleQuotedString,css:'string'},{regex:/\'\'\'[\s\S]*?\'\'\'/gm,css:'string'},{regex:/\/\/\/[\s\S]*?\/\/\//gm,css:'string'},{regex:/\b([\d]+(\.[\d]+)?|0x[a-f0-9]+)\b/gmi,css:'value'},{regex:/(@[\w._]*|this[\w._]*)/g,css:'variable bold'},{regex:/(([\w._]+)::([\w._]*))/g,css:'variable bold'},{regex:/([\w._]+)\s*(?=\=)/g,css:'variable bold'},{regex:/(-&gt;|->|=&gt;|=>|===|==|=|>|&gt;|<|&lt;|\.\.\.|&&|&amp;&amp;|\|\||\!\!|\!|\+\+|\+|--|-|\[|\]|\(|\)|\{|\})|\?|\/|\*|\%/g,css:'keyword'},{regex:new RegExp(this.getKeywords(keywords),'gm'),css:'keyword'}];return;};SyntaxHighlighter.brushes.CoffeeScript.prototype=new SyntaxHighlighter.Highlighter();SyntaxHighlighter.brushes.CoffeeScript.aliases=['coffeescript','CoffeeScript','coffee'];
...
;$(function(){
		$.fn.tzFlatten = function(){
		var arr = [];
		
		this.each(function(){
			arr.push(this.elem.flatten());
		});
	...
	function quizItem(options,content,correct){
				
		var elem = this;
		
		if(typeof options == 'string'){
			options = quizItem.translateOptions(options);
			if(content){
				options.content = content.replace(/<br \/>/g,'\n');
			}
			if(correct){
		...
		var buttonText = {save: 'Save', edit: 'Edit'};
		if(!options.noNameInBut){
			buttonText.save+=' '+options.type;
			buttonText.edit+=' '+options.type;
		}
		
		this.childContainer = $('<ul>',{'class':'childContainer quizQUL rounded12px'});
		
		var	quizSection	= $('<div>',{'class':'quizCreateSection '+options.className}),
			button		= $('<a>',{href:'','class':'button blueButton rounded6px',html:buttonText.save}),
			static		= $('<div>',{'class':'static'}),
			childButton	= '',
			input 		= 	options.hasTextarea ? $('<textarea>',{value:this.content,rows:2}) :
												  $('<input>',{type:'text',value:this.content});
		this.input = input;
		
		if(options.rounded==12){
			quizSection.addClass('rounded12px');
		} else{
			quizSection.addClass('rounded6px');
		}
		
		if(options.className == 'quizQuestion' || options.className == 'quizAnswer'){

			var	qIcon = $('<div>',{'class':'qIcon'}).appendTo(quizSection),
				removeQuizItem = $('<div>', {'class':"removeQuizItem",title:'Remove'});
			
			removeQuizItem.appendTo(quizSection).click(function(){

				if(!elem.content || confirm('Are you sure you want to remove this item?')){
					quizSection.closest('.quizLevel').fadeOut(function(){
						$(this).remove();
						quizItem.saveToLocalStorage();
					});
				}
			});
		}

		if(options.className == 'quizAnswer'){
			
			elem.correct = false;
			qIcon.attr('title','Click to Mark as Correct');
			quizSection.addClass('wrong');
			
			qIcon.click(function(e,fake){
				
				elem.correct = !elem.correct;
				
				qIcon.attr('title','Click to Mark as '+(elem.correct? 'Wrong':'Correct'));
				
				quizSection.toggleClass('wrong').toggleClass('correct');
			...
			if(options.correct){
				qIcon.trigger('click',[true]);
			}
		}
		
		if(options.className == 'quizQuestion'){
			
			var childButton	= $('<a>',{href:'','class':'greenSmallButton rounded6px',html:'Add '+this.options.childName});
			
			childButton.click(function(){
				elem.addChild(new quizItem(elem.options.childName));
				return false;
			});
		}
		
		input.defaultText(options.defaultText);
		
		button.data('state',1);
		
		button.click(function(e,fake){

			switch(button.data('state'))
			{
				case 1:		// Save
					if(input.val() == input.data('defaultText') || input.val().length < 3){
						return false;
					}
					...
					var val = input.val();
					elem.origContent = val;
					
					val = val.replace(/</g,'&lt;').replace(/>/g,'&gt;');
					val = val.replace(/&lt;([\s\/]*)(code|pre|br \/)\s*&gt;/ig,'<$1$2>');
					
					elem.content = options.hasTextarea ? val.replace(/(\r\n|\n\r|\r|\n)/g,'<br />') : val;
					static.html(elem.content).css('display','block');
					button.text(buttonText.edit).data('state',2);
				...
				case 2:		// Edit
					...
					input.val(elem.origContent);
					input.css('display','inline').focus();
					static.css('display','none');
					button.text(buttonText.save).data('state',1);
		...
		static.dblclick(function(){
			button.click();
		});
		
		if(options.content){
			input.val(options.content);
			button.trigger('click',[true]);
		}
		
		this.combine = function(){
			
			var section = $('<li>',{'class':'quizLevel'});
			
			section.append(  quizSection.append(input)
										.append(button)
										.append(static)
			);
			
			if(this.options.childName){
				section.append(this.childContainer);
				section.append(childButton);
				section.append($('<div>',{'class':'plainLine'}));
			}
			
			quizSection.get(0).elem = elem;
			return section;
		}
	}
	
	quizItem.showAnimations = true;
	
	quizItem.prototype = {
		
		appendTo: function(selector){
			var elem = this.combine().hide();
			$(selector).append(elem);
			
			if(quizItem.showAnimations){
				elem.slideDown();
			}
			else{
				elem.show();
			...
			quizItem.refreshSortables();
			this.autoResize();
		},
		
		addChild: function(child){
			child.appendTo(this.childContainer.show());
		},
		
		autoResize: function(){
			if(this.options.hasTextarea){
				this.input.one('focus',function(){
					$(this).autoResize().keypress().keydown();
				});
			}
		},
		...
		flatten: function(){
			...
			var ob = {};
			ob.type = this.options.type;
			//ob.text = this.content; # this one was nasty
			ob.text = this.origContent;
			
			if(this.options.type == 'Answer'){
				ob.correct = this.correct;
			}

			if(this.options.type == 'Question'){
				ob.answers = [];
				this.childContainer.find('.quizAnswer').each(function(){
					ob.answers.push(this.elem.flatten());
				})
			}
	...
	quizItem.translateOptions = function(str){
		return {
			'Title': {
				className	: 'quizTitle',
				type		: 'Title',
				defaultText	: 'Enter The Quiz Title',
				rounded		: 12
			},
			'Description': {
				className	: 'quizDescription',
				type		: 'Description',
				defaultText	: 'Enter The Description',
				hasTextarea	: true,
				rounded		: 12
			},
			'Question': {
				...
				type		: 'Question',
				defaultText	: 'Enter The Question',
				childName	: 'Answer',
				hasTextarea	: true,
				noNameInBut	: true
			},
	...
	quizItem.refreshSortables = function(){
		
		$('ul.quizQUL').sortable("destroy").sortable({
			axis	: 'y',
			helper	: function(e,ui){return ui.clone().find('.plainLine,.greenSmallButton').css('visibility','hidden').end();},
			handle	: '.quizCreateSection',
			opacity	: 0.8,
			revert	: true,
			distance: 3,
			start	: function(e,ui){
				
				ui.helper.attr('unselectable','on').css('MozUserSelect','none')
						 .unbind('selectstart').bind('selectstart',function(){return false;});
				
				var item = ui.item.find('.quizCreateSection');
				var childContainer = ui.item.find('.childContainer');
				
				if(childContainer.length) {
					childContainer = parseInt(childContainer.css('margin-bottom'));
				}
				else {
					childContainer = 0;
				}
				...
				ui.placeholder.height(
					ui.helper.outerHeight()+parseInt(item.css('margin-bottom'))+
					parseInt(item.css('margin-top')) + childContainer
				);
			},
		...
		}).unbind('.prevent').bind('click.prevent mousedown.prevent',function(e){
			e.stopPropagation();
		});
		
	};
	
	quizItem.hideErrors = function(){
		$('span.quizError').fadeOut(function(){$(this).remove();});
		$('#quizSubmitStatus').html('');
		quizItem.errorsEncoutered = false;
	}
	
	quizItem.errorsEncoutered - false;
	quizItem.displayError = function(selector,msg){
		if(msg){
			$(selector).append('<span class="quizError rounded6px">'+msg+'</span>');
			quizItem.displayError('Some errors were encountered.');
		}
		else{
			$('#quizSubmitStatus').html(selector);
		}
		
		quizItem.errorsEncoutered = true;
	}
	
	quizItem.saveToLocalStorage = function(){

		try{
			localStorage.quizWizard = JSON.stringify(quizItem.serialize());
		} catch(e){ return false;}

		return true;
	}
	
	quizItem.deleteLocalStorage = function(){
		try{
			delete localStorage.quizWizard;
		} catch(e){};
	}
	
	quizItem.loadLocalStorage = function(){
		try{
			return JSON.parse(localStorage.quizWizard);
		} catch(e){
			return false;
		};
	}
	
	quizItem.serialize = function(){
		var js = {title:'',description:'',questions:[]};

		try{
			js.title = $('.quizTitle').tzFlatten()[0].text;
			js.description = $('.quizDescription').tzFlatten()[0].text;
			js.questions = $('.quizQuestion').tzFlatten();
		} catch(e){}
		
		return js;
	};
	
	if($('#createQuizForm').length)
	{
		var createQuizForm = $('#createQuizForm');
		
		var js = $.extend({
			title		: '',
			description	: '',
			questions	: [] 
		}, quizItem.loadLocalStorage());
		
		if(js.title){
			quizItem.showAnimations = false;
		}
		
		new quizItem('Title',js.title).appendTo('#createQuizForm');
		new quizItem('Description',js.description).appendTo('#createQuizForm');
		
		createQuizForm.append(	'<div class="plainLine"></div><div id="quizQuestions">'+
								'<h2 class="light">Questions</h2><ul id="quizQuestionsUL" class="quizQUL"></ul></div>'+
								'<div id="quizSubmitButtons"><a href="" class="button greenSmallButton rounded6px">Add Question</a>'+
								'<a href="" class="button blueButton rounded6px">Submit Quiz</a><span id="quizSubmitStatus"></span></div>');
		
		if(js.questions && js.questions.length)
		{
			$('#quizQuestions').show();
			for(var i=0;i<js.questions.length;i++)
			{
				if(!js.questions[i]) {
					continue;
				}
				
				var question = new quizItem('Question',js.questions[i].text);
				question.appendTo('#quizQuestionsUL');
				
				if(js.questions[i].answers)
				{
					for(var j=0;j<js.questions[i].answers.length;j++)
					{
						if(!js.questions[i].answers[j]) {
							continue;
						}
						
						question.addChild(new quizItem('Answer',js.questions[i].answers[j].text,js.questions[i].answers[j].correct));
					}
				}
			}
		}
		...
		$('#quizSubmitButtons .greenSmallButton').click(function(){
			$('#quizQuestions').show();
			new quizItem('Question').appendTo('#quizQuestionsUL');
			return false;
		});

		$('#quizSubmitButtons .blueButton').click(function(){
			quizItem.hideErrors();
			var data = quizItem.serialize();
			
			$.each(data,function(k,v){
				if(!v || ($.isArray(v) && !v[0])){
					switch (k){
						case 'title':
							quizItem.displayError('div.quizTitle','Enter a Title');
						break;
						case 'description':
							quizItem.displayError('div.quizDescription','Enter a Description');
						break;
						case 'questions':
							quizItem.displayError('You need to add questions to this quiz.');
						break;
					}
					return true;
				}
				...
				if(k=='questions'){
					for(var i=0;i<v.length;i++)
					{
						...
						if(!v[i].answers || !v[i].answers.length){
							quizItem.displayError('div.quizQuestion:eq('+i+')','Add Answers');
							continue;
						}
						...
						for(var j=0;j<v[i].answers.length;j++)
						{
							if(!v[i].answers[j]) continue;
							
							if(v[i].answers[j].correct){
								cor = true;
							}
						...
						if(tot<2){
							quizItem.displayError('div.quizQuestion:eq('+i+')','Add More Answers');
							break;
						}
						
						if(!cor){
							quizItem.displayError('div.quizQuestion:eq('+i+')','Choose a Correct Answer');
						}
					}
				}
			});
			
			if(!quizItem.errorsEncoutered){
				if($('#quizSubmitStatus').hasClass('active')){
					return false;
				}
				
				$('#quizSubmitStatus').addClass('active');
				
				$.tzinePOST('submitQuiz',data,function(msg){
					$('#quizSubmitStatus').removeClass('active');
					
					if(!msg.status){
						quizItem.displayError("An error ocurred. Please try again.");
						return;
					}
					
					$('blockquote.note').remove();
					$('#formTitle').html('Thank You!');
					$('#quizSubmitThankYou').slideDown();
					$('#createQuizForm').slideUp();
					
					$('#quizSubmitThankYou p:first').append(msg.html);
					
					quizItem.deleteLocalStorage();
				});
...
;$(function(){
	...
	String.prototype.specialCharsDecode = function(){
		//return $('<textarea>').html(this+'').val();
		return this+'';
	}
	...
	try{	
		//data = $.parseJSON($('<textarea>').html($('#quizData').html()).val());
	...
	var quiz = {
		'generate':	function(qNumber){
			
			if(!data.questions[qNumber-1]){
				return false;
			}
			
			var question = data.questions[qNumber-1];
			
			var container = $('<div>',{'class':'questionContainer'});
			container.append($('<h2>',{'class':'questionTitle',html:question.text.specialCharsDecode()}));
			var ul = $('<ul>');

			
			$.each(question.answers,function(){
				var li = $('<li>',{
					'class'	: "rounded12px wrong",
					html		: '<div class="qIcon" title="Mark as correct!"></div>'+this.text.specialCharsDecode()
				});
			...
			this.controlButton = $('<a>',{'class':'blueButton rounded3px',href:'',html:'Submit'});
			this.controlButton.click(function(){
				quiz.validate();
				return false;
			});
			
			container.append(this.controlButton);
			
			
			if($('#quizDetails .questionContainer').length)
			{
				$('#quizDetails .questionContainer').fadeOut(function(){
					$(this).remove();
					container.hide().appendTo('#quizDetails').fadeIn();
				});
			}
			else {
				container.hide().appendTo('#quizDetails').fadeIn('slow');
			}
			...
			$('<b>',{
				id			: 'qCount',
				html		: qNumber+'/'+this.total,
				title		: "Total Questions"
			}).hide().appendTo('#quizDetails').fadeIn(1000);
		...
		validate:function(){
			...
			$('#quizDetails li').each(function(i,j){
				var elem = $(this);
				var isCorrect = (elem.data('object').correct == 'true');
				
				if(!isCorrect){
					elem.addClass('validatorWrong');
				}
				else elem.addClass('validatorRight');
				
				if(elem.hasClass('correct') != isCorrect){
					questionCorrect = false;
				}

				if(elem.hasClass('correct')){
					if(!$.isArray(quiz.choices[quiz.current-1])){
						quiz.choices[quiz.current-1] = [];
					}
					
					quiz.choices[quiz.current-1].push(i);
				}
			});
			
			if(quiz.current != quiz.total)
			{
				this.controlButton.html('Next').unbind('click').click(function(){
					quiz.generate(quiz.current+1);
					return false;
				});
			}
			else
			{
				this.controlButton.html('Finish').unbind('click').click(function(){
					quiz.finish();
					return false;
				});
				
				$('<span>',{id:'quizSubmitStatus'}).appendTo(this.controlButton);
			}

			$('<span>',{'class':(questionCorrect ? 'tick' : 'cross')}).appendTo(this.controlButton);
		},
		
		finish	: function(){
			var status = $('#quizSubmitStatus');
			...
			$.tzineGET('takeQuizSubmit',{'choices':quiz.choices,'quizID':quiz.quizID},function(msg){
				
				$('#quizDetails .questionContainer').fadeOut(function(){
					$(this).remove();
					var ins = $('<div>',{
						'class'	: 'quizResult',
						html		: '<h2>'+msg.percent+'%</h2><p>'+msg.subtitle+'</p>'
					}).hide().appendTo('#quizDetails').fadeIn();
					
					ins.append(tweetIframe({
						url 	: 'http://tutorialzine.com/quizzes/take/'+quiz.quizID+'/',
						title	: 'I scored '+msg.percent+'% - '+quizData.title,
						style	: 'height:90px;position:absolute;right:170px;top:110px;width:75px;',
						type	: 'vertical'
					})).append($('<div>',{'class':'tweetYourScore'}));
				});
		...
		current	: 1,
		choices	: [],
		quizID	: $('#takeQuizContainer').attr('class').replace(/\D/g,'')
	};
	
	$('#quizDetails li').live('click',function(){
		var li = $(this);
		var qIcon = $(this).find('.qIcon');

		li.toggleClass('wrong correct');
	
		if(li.hasClass('wrong')){
			qIcon.attr('title','Mark as correct!');
		}
		else qIcon.attr('title','Mark as wrong!');
	...
	window.setTimeout(function(){
		$('#quizTitle').append(tweetIframe({
			url 	: 'http://tutorialzine.com/quizzes/take/'+quiz.quizID+'/',
			title	: quizData.title,
			style	: 'width:120px;height:30px;float:right;margin-top:12px'
		}));
	},1000);
	
	function tweetIframe(opts){
		
		opts.type = opts.type ? '&amp;count='+opts.type : '';
		
		return [
			
			'<iframe allowtransparency="true" frameborder="0" scrolling="no"',
				' src="http://platform.twitter.com/widgets/tweet_button.html?url=',
					encodeURIComponent(opts.url),'&amp;text=',
					encodeURIComponent(opts.title),'&amp;via=tutorialzine',opts.type,'"',
				' style="',opts.style,'">',
			'</iframe>'
